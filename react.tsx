...
const PasswordManager = () => {
  ...
  const generatePassword = useCallback(async (site) => {
  ...
  const renderWelcomeScreen = () => (
    <div className="grid w-full items-center gap-4">
      ...
      <Button onClick={handleEnter} disabled={!passphrase}>Enter</Button>
    </div>
  );
  ...
  const renderMainScreen = () => (
    <div className="grid w-full items-center gap-4">
      ...
      <div className="fixed bottom-4 right-4 flex space-x-2">
        ...
        <Button onClick={() => {setEditingSite(null); setNewSite({ email: '', domain: '', rotationRounds: 1 }); setScreen('addSite');}} size="icon">
          <Plus size={24} />
        </Button>
      </div>
    </div>
  ...
  const renderAddSiteScreen = () => (
    <div className="grid w-full items-center gap-4">
      ...
      <Button onClick={() => { editingSite !== null ? updateSite() : addSite(); setScreen('main'); }}>
        {editingSite !== null ? 'Update Site' : 'Add Site'}
      </Button>
      <Button onClick={() => {setNewSite({ email: '', domain: '', rotationRounds: 1 }); setEditingSite(null); setScreen('main');}} variant="outline">Cancel</Button>
    </div>
  );
  ...
  const renderSettingsScreen = () => (
    <div className="grid w-full items-center gap-4">
      ...
      <Button onClick={() => setScreen('main')} variant="outline">Back to Main</Button>
    </div>
  );

  return (
    <div className="flex justify-center items-center min-h-screen bg-gray-100">
      <Card className="w-full max-w-[600px] m-4">
        <CardHeader>
